...
def calculate_metrics_ann(results, cv, name=None):
    ...
    Parameters:
        ...
        cv (bool): Indicate if metrics are calculated for a cross-validation setting (multiple folds) or not
...
def ann(x_train, y_train, smp_idx_train, ann_type="lstm", name="LSTM", cv_timeseries=0.2, plot_loss=False, plot_loss_name=None, **params):
    ...
    if isinstance(cv_timeseries, float):
        ...
        for k in tqdm(cv_timeseries):
            ...
            # all_results["score_time"].append(k_results["score_time"])
            # all_results["fit_time"].append(k_results["fit_time"])
            # all_results["y_true_train"].append(k_results["y_true_train"])
            # all_results["y_true_valid"].append(k_results["y_true_valid"])
            # all_results["y_pred_train"].append(k_results["y_pred_train"])
            # all_results["y_pred_valid"].append(k_results["y_pred_valid"])
            # all_results["y_pred_prob_train"].append(k_results["y_pred_prob_train"])
