...
from snowdragon.utils.helper_funcs import load_configs, load_main_config, load_smp_data

# checked
from snowdragon.visualize.data.all_profiles import all_in_one_plot, bog_plot

# checked
from snowdragon.visualize.data.dataset import (
    anova,
    corr_heatmap,
    pairwise_features,
    plot_balancing,
...
# checked
from snowdragon.visualize.data.profile import smp_features, smp_labelled, smp_unlabelled

# checked
from snowdragon.visualize.explainability.decision_tree import (
    forest_extractor,
    visualize_tree,
)

# unchecked
from snowdragon.visualize.results.plot_results import (
...
EVAL_LOC = load_main_config("main_config.yaml")["processed_data"]["evaluation"]
SMP_ORIGINAL_NPZ = load_main_config("main_config.yaml")["processed_data"]["npz_file"]
SMP_NORMALIZED_NPZ = load_main_config("main_config.yaml")["processed_data"]["normalized_npz_file"]
SMP_PREPROCESSED_TXT = load_main_config("main_config.yaml")["processed_data"]["preprocessed_data"]
    ...
    **kwargs,
    ...
    if plot_smp_features:
        ...
        smp_features(
            ...
            file_name=store_path / (f"{str(example_smp_name)}_features.png"),
        )
    ...
    **kwargs,
    ...
    if plot_one_unlabelled_smp:
        ...
        smp_unlabelled(
            smp,
            smp_name=example_smp_name,
            file_name=store_path / (f"{str(example_smp_name)}_unlabelled.png"),
        )
    ...
    if plot_one_labelled_smp:
        ...
        smp_labelled(
            ...
            anti_labels=anti_labels,
            smp_name=example_smp_name,
            file_name=store_path / (f"{str(example_smp_name)}_labelled.png"),
        )
    ...
    if plot_bog_plot:
        print("\t\tPlotting bog plot ...")
        bog_plot(smp, file_name=store_path / "bog_plot.png")
    ...
    if cf_matrix:
        ...
        for i, group in enumerate([group_1, group_2, group_3]):
            ...
            plot_confusion_matrix(
                ...
                label_orders_group,
                names_group,
                file_name=f"{store_path / 'confusion_matrixes_' + str(i)}.pdf",
            )
...
def main():
    ...
    if args.normalized_data:
        visualize_normalized_data(smp_preprocessed)
    ...
    if args.tree:
        ...
        visualize_tree(
            ...
            file_name=str(f"{OUTPUT_DIR / 'decision_tree'}_{str(tree_idx)}"),
            format="svg",
        )
    ...
    if args.results:
        ...
        visualize_results(
            all_scores, label_acc, label_prec, cf_matrix=True, roc_auc=False, bog_plot=False, comparison=False
        )
    ## PREDICTIONS ##
