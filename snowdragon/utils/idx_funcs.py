...
def idx_to_int(string_idx):
    ...
    if "PS122" in string_idx:
        ...
        return int(f"1{str_parts[1]}{str_parts[2].zfill(2)}{str_parts[3].zfill(3)}")
    # SMP naming convention
        ...
        try:
            ...
            raise ValueError(
                "Device {string_idx} was not found. It looks like you are using the traditional SMP file naming convention. If yes, please add your SMP device (start of filename) to the configs/smp_devices/smp_devices.yaml."
            )
        ...
        try:
            ...
            raise RuntimeError(
                "Something is probably wrong with the code - this is not your fault (except if you are writing the code)."
            )
    ...
    # Please make sure to add it to the configs/smp_devices/smp_devices.yaml
        ...
        raise ValueError(
            "SMP naming convention is unknown. Please add another elif line in idx_to_int to handle your SMP naming convention."
        )
...
def int_to_idx(int_idx: int) -> str:
    ...
    if (smp_device == 1) or (smp_device == 0):
        raise ValueError(
            "SMP indices with 0 or 1 cannot be converted. Indices with 1 are reserved for event IDs. 0 means that no suitable match was found during index convertion."
        )
